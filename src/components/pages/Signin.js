...
function Signin() {
  const { setIsLoggedIn } = useContext(ImgurifyContext);
  ...
  const submitForm = (e) => {
    ...
    if (user.username === "" || user.password === "") {
      ...
      setErr(false);
      axios
        .post("http://localhost:3007/admin-login", user)
        .then((resp) => {
          if (resp.data.msg === "Login successful") {
            localStorage.setItem("Imgur_AdminUSER", JSON.stringify(resp.data));
            setIsLoggedIn(true);
    ...
    <div className="reg-bk">
      ...
      <div className="row justify-content-center text-white">
        <div className="col-md-6">
          <div className="">
            <div className="reg-text text-center pt-5">
              <h2>Imgurify Admin Login</h2>
            </div>
            <div className="form-heading text-center mt-5">
              <h2 className="pt-5 form-heading2">Sign In with</h2>
            </div>

            <div className="imgur-icons">
              <div className="grid-5 row">
                <div className="sc-bk col">
                  <FaFacebook
                    className="imgur-social-icons"
                    style={{ fontSize: "20px" }}
                  />
                </div>
                <div className="sc-bk col">
            ...
            <form className="form-content" onSubmit={submitForm}>
              ...
              <div className="form-btn2">
                <div>
                  <Link to="/signup" className="newpost">
                    <p>need an account?</p>
                  </Link>
                </div>
